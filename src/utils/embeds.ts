...
export function buildDenialEmbed(refId: string, reasons: string): EmbedBuilder {
  return base(config.colours.denial)
    ...
    .setDescription(`Your submission could not be approved.\n\n**Reasons:**\n${reasons}`)
...
export function buildProofRequestEmbed(refId: string, type: 'ownership' | 'funds'): EmbedBuilder {
  const desc = type === 'ownership'
    ? 'Additional ownership verification is required before this submission can proceed.\n\nPlease submit a short video showing your project files and creation process.'
    : 'Additional funds verification is required before this submission can proceed.\n\nPlease submit a screen recording showing your available funds and a page refresh.';
...
export function buildModerationHoldEmbed(refId: string): EmbedBuilder {
  return base(config.colours.moderation)
    .setTitle('Post On Hold')
    .setDescription('Your post has been placed on hold for further review.')
    .setFooter({ text: `${FOOTER} | ${refId}` });
}
